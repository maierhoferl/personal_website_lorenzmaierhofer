#!/usr/bin/env python3
"""
Grimasso Referral Kit PDF Generator — German (DE)
...
OUTPUT_PATH = os.path.join(SCRIPT_DIR, "..", "de", "fuer-logopaedinnen", "grimasso-empfehlungskarte-de.pdf")
...
def generate(fonts):
    ...
    c.setTitle("Grimasso – Empfehlungskarte für Therapeuten")
    c.setAuthor("Lorenz Maierhofer")
    c.setSubject("Grimasso Ihren Patienten empfehlen")
    ...
    label_y = HDR_TOP - 26
    draw_spaced(c, "VON IHRER LOGOPÄDIN",
    ...
    c.drawString(MARGIN, label_y - 32, "Die Hausaufgaben Ihres Kindes")
    c.drawString(MARGIN, label_y - 58, "— Mit Spaß gemacht")
    c.restoreState()
    ...
    sub_y = div_y - 16
    c.drawString(MARGIN, sub_y, "Kostenlos · 7–12 Jahre · iOS")
    c.drawString(MARGIN, sub_y - 15, "Empfohlen von Ihrer Therapeutin")
    c.restoreState()
    ...
    draw_spaced(c, "WAS IST GRIMASSO?", MARGIN, sec_y, fonts["extrabold"], 7, color=LABEL_CLR)
    body = ("Grimasso verwandelt die Zungen-, Lippen- und Kieferübungen, die Ihre Therapeutin verschrieben hat, "
            "in ein Spiel, das Ihr Kind spielen möchte. 100 angeleitete Übungen auf 20 Levels — mit einem niedlichen "
            "Frosch-Maskottchen, Streaks und Abzeichen, die tägliches Üben wie eine Errungenschaft anfühlen lassen.")
    wrap_text(c, body, MARGIN, sec_y - 14, W - 2*MARGIN, fonts["regular"], 9, 13.5, BODY_TEXT)
    draw_sep(WHAT_BOT - GAP // 2)

    # ────────────────────────────────────────────────────────────────────────
    # WAS IHR KIND ÜBEN WIRD
    # ────────────────────────────────────────────────────────────────────────
    sec_y = CAT_TOP - LPAD
    draw_spaced(c, "WAS IHR KIND ÜBEN WIRD", MARGIN, sec_y, fonts["extrabold"], 7, color=LABEL_CLR)

    categories = ["Zungenspitze", "Zungenstärke", "Zungenkontrolle", "Schlucken", "Kiefermuskulatur"]
    ...
    # ────────────────────────────────────────────────────────────────────────
    # WAS SIE ALS ELTERNTEIL ERHALTEN  (2-column checkmark grid + social proof strip)
    # ────────────────────────────────────────────────────────────────────────
    sec_y = CLP_TOP - LPAD
    draw_spaced(c, "WAS SIE ALS ELTERNTEIL ERHALTEN", MARGIN, sec_y, fonts["extrabold"], 7, color=LABEL_CLR)

    points = [
        ("Dauerhaft kostenlos",     "Vollzugang, kein Abo, keine versteckten Kosten"),
        ("Eltern-Dashboard",        "Sehen Sie, was Ihr Kind täglich geübt hat"),
        ("Keine Werbung, keine Daten", "Kindgerecht — DSGVO & COPPA konform"),
        ("Spiegel-Kamera",          "Das Kind sieht sich bei jeder Übung selbst"),
        ("Tägliche Erinnerungen",   "Die App motiviert — ohne dass Sie mahnen müssen"),
        ("Mehrere Kinderprofile",   "Funktioniert für alle Ihre Kinder in einer App"),
        ("Sprachanleitungen",       "Klare Audioführung durch jede Übung"),
        ("20 progressive Level",    "Hält die Motivation Woche für Woche hoch"),
        ("Offline nutzbar",         "Kein WLAN nötig nach der Installation"),
    ]
    ...
    c.setFillColor(GREEN_DARK)
    c.drawString(MARGIN + 14, strip_title_y, "Kinder bleiben motiviert — weil es sich wie ein Spiel anfühlt")
    c.restoreState()

    # Strip body
    strip_body = ("Grimasso nutzt denselben Motivationskreislauf wie Lieblingsspiele von Kindern: Levels zum Meistern, "
                  "Abzeichen zum Sammeln und eine Figur, die jeden Erfolg feiert. Kinder, die Grimasso nutzen, üben "
                  "regelmäßiger und selbstständiger — was Ihren Alltag leichter macht.")
    ...
    stats = [("10", "Spiellevel"), ("25+", "Erfolgsabzeichen"), ("100", "angeleitete Übungen")]
    chip_h = 22
    chip_y = SP_BOT + 10
    chip_gap = 10
    c.saveState()
    ...
    # ────────────────────────────────────────────────────────────────────────
    sec_y = HOW_TOP - LPAD
    draw_spaced(c, "IN 3 SCHRITTEN LOSLEGEN", MARGIN, sec_y, fonts["extrabold"], 7, color=LABEL_CLR)

    steps = [
        ("1", "Kostenlos laden",  "Suchen Sie im\nApp Store nach\n\"Grimasso\"."),
        ("2", "Kind einrichten",  "Profil anlegen und\nÜbungsprogramm\nin 2 Minuten wählen."),
        ("3", "Heute spielen",    "Erstes Level\ngemeinsam machen —\nnur 5 Minuten."),
    ]
    ...
    c.setFillColor(GREEN_DARK)
    c.drawString(cta_x, cta_top_y - 15, "Heute kostenlos herunterladen")
    c.restoreState()

    c.saveState()
    c.setFont(fonts["regular"], 8.5)
    c.setFillColor(MID_GRAY)
    wrap_text(c, "Suchen Sie nach \"Grimasso\" oder scannen Sie den QR-Code mit Ihrer iPhone-Kamera.",
    ...
    c.setFont(fonts["bold"], 9.5)
    c.setFillColor(WHITE)
    lbl = "Im App Store herunterladen"
    ...
    c.setFont(fonts["regular"], 7.5)
    c.setFillColor(MID_GRAY)
    contact_text = "Fragen? grimasso@lorenzmaierhofer.com"
...
if __name__ == "__main__":
    print("Grimasso Empfehlungskarte Generator (DE)")
    print("=========================================")
    print("Loading fonts…")
    fonts = load_fonts()
